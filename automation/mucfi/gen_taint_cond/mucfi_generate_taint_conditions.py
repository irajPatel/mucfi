...
def yosys_cellift_and_find_fwd_mux(config, y_log_dir, cellift_find_mux_log, taint_tracking_method, y_script, design_file):
    ...
    with open(Path(os.path.realpath(__file__)).parents[0] / "yosys_templates" / y_script, "r") as f:
        ...
        with open (yscript_mod, "w") as f:
            f.write(yscript)
        yosys_bin = str(Path(config['YOSYS_DIR']) / 'yosys')
        result = run_cmd(f"{yosys_bin} -c {yscript_mod} > {cellift_find_mux_log}", print_stdout=False)
...
def cellift_and_declassify(config, config_file, taint_tracking_method):
    ...
    tt_method = config['TAINT_TRACKING_METHOD']
    design_file = str(d_out_dir / f"cell{tt_method}.v")
    design_file_extra_mux = str(d_out_dir / f"cell{tt_method}_extra_mux.v")
...
def yosys_gen_uc(config, tcl_name, uc_call, design_with_uc, uc_log):
    ...
    yosys_bin = str(Path(config['YOSYS_DIR']) / 'yosys')
    run_cmd(f"{yosys_bin} -c {uc_tcl_file} > {uc_log}", print_stdout=False)
